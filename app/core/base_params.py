...
class PaginationQueryParam:
    ...
    def __init__(
        self,
        pageNum: int = Query(default=1, description="当前页码", ge=1, alias="pageNum"),
        pageSize: int = Query(default=10, description="每页数量", ge=1, le=100, alias="pageSize"),
        ...
        参数:
        - pageNum (int | None): 当前页码，默认 None。
        - pageSize (int | None): 每页数量，默认 None，最大 100。
        - orderBy (str | None): 排序字段，格式 'field,asc;field2,desc'。
        ...
        # 将字符串格式的orderBy转换为服务层需要的List[Dict[str, str]]格式
        if orderBy:
            try:
                self.order_by = json.loads(orderBy)
            except ValueError:
                # 如果解析失败，使用默认排序
