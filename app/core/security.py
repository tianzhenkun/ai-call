...
def decode_access_token(token: str) -> "JWTPayloadSchema":
    ...
    try:
        ...
        # 处理payload
        # 实际格式包含: userId, userName, tenantId 等字段
        ...
        # 构建符合系统格式的session_info
        session_info = {
            "session_id": f"ruoyi_{user_id}",
            "user_id": int(user_id),
            ...
            "os": "Unknown",
            "browser": "Unknown",
            "login_time": payload.get("iat", 0),
            "login_type": "jwt",
        }

        # 构建新的payload，使用json.dumps()确保JSON格式
        new_payload = {
            "sub": json.dumps(session_info),  # 使用JSON格式
            "is_refresh": False,
            "exp": payload.get("exp", int((datetime.now() + timedelta(hours=24)).timestamp())),
        }
        from app.api.v1.system.auth.schema import JWTPayloadSchema
