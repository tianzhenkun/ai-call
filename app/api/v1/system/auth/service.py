...
class LoginService:
    ...
    async def create_token_service(
        ...
        access_expires = timedelta(hours=24)  # 访问令牌有效期24小时
        refresh_expires = timedelta(days=7)  # 刷新令牌有效期7天
...
class AutoLoginService:
    ...
    async def get_auto_login_users_service(cls, db: AsyncSession) -> list[AutoLoginUserSchema]:
        ...
        # 查询所有启用的用户
        stmt = select(UserModel).where(UserModel.status == "0").order_by(UserModel.user_id)
