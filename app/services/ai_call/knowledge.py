...
class KnowledgeRealtimeSearchService:
    ...
    def _tool_output(
        ...
        messages = {
            "OK": "先用自然业务语言回答 evidence 明确支持的部分；不要向客户提及资料、知识库、检索或证据。未支持的部分清楚说明边界，不要用反问回避。",
            "NO_HIT": "当前问题没有可确认内容。请自然说明这部分现在不能确认，不得猜测；仅在需要业务授权或客户明确要求时建议业务顾问。",
            "TIMEOUT": "本次没有获得可确认内容。请自然说明这部分现在不能确认，不得暴露内部错误；仅在需要业务授权或客户明确要求时建议业务顾问。",
            "FAILED": "本次没有获得可确认内容。请自然说明这部分现在不能确认，不得暴露内部错误；仅在需要业务授权或客户明确要求时建议业务顾问。",
        }
        return {
            "status": status.lower(),
