...
class RealtimeCallAgentRunner:
    ...
    async def _handle_user_speech_started(
        ...
        if not self._is_barge_in_enabled_for_session(session):
            ...
            should_continue_overlap_turn = (
                self._should_continue_no_barge_overlap_turn(call_id)
            )
            turn = self._pending_turn(
                call_id,
                reset_if_finished=not should_continue_overlap_turn,
            )
            if not self._has_active_model_response(call_id):
                turn.no_barge_overlap_stopped_during_ai_response = False
            self._cancel_turn_response_task_nowait(call_id)
            if turn.stopped_at is not None and (
                not turn.response_requested or should_continue_overlap_turn
            ):
                turn.stopped_at = None
            if turn.transcript and (
                not turn.response_requested or should_continue_overlap_turn
            ):
                turn.transcript_merge_start_index = len(turn.transcript_parts)
            if should_continue_overlap_turn:
                self._append_event(
                    call_id,
                    "no_barge_overlap_turn_continued",
                    "agent",
                    {"reason": "response_already_requested_during_ai"},
                )
    ...
    async def _handle_tool_call_done(
        ...
        if not self._accepts_call_end_tool(call_id, tool_reason):
            rejection_reason = self._call_end_tool_rejection_reason(tool_reason)
            self._append_event(
                call_id,
                "call_end_tool_ignored",
                "agent",
            ...
            try:
                await provider.submit_tool_result(
                    tool_call_id,
                    self._call_end_rejected_tool_result(tool_reason),
                )
    ...
    @staticmethod
    def _call_end_rejected_tool_result(tool_reason: str) -> str:
        if tool_reason == "task_completed":
    ...
    async def _complete_response_and_flush_pending(
        ...
        lifecycle.cancel_pending = False
        guard.cancel_requested = False
        self._split_unmaterialized_no_barge_overlap_turn_if_needed(call_id)
        if cancel_was_pending:
            self._mark_provider_cancel_race_window(lifecycle)
        if not lifecycle.pending_create:
            if await self._maybe_schedule_deferred_no_barge_turn(call_id, provider):
                return
