...
@pytest.mark.anyio
async def test_realtime_tool_uses_frozen_scope_and_keeps_evidence_out_of_events() -> None:
    context = parse_knowledge_runtime_context(
        _snapshot(),
        tenant_id="tenant-a",
        task_id=7,
    ...
    assert service.search_calls[0]["context"] == context
    assert service.search_calls[0]["query"] == "退款审核后多久能到账？"
    ...
    result_event = next(
        ...
        if event.type == "knowledge_tool_result"
    )
    ...
    assert runner._event_payload(
        "tool_call_done",
        {
            "name": "search_scene_knowledge",
            "arguments": '{"query":"客户原话"}',
        },
    )["arguments"] == "<redacted_knowledge_query>"

    no_context = CallSession(
        call_id="call-2",
        room_name="room-2",
    ...
    assert SEARCH_SCENE_KNOWLEDGE_TOOL not in runner._session_config(no_context).tools


@pytest.mark.anyio
async def test_realtime_search_falls_back_on_no_hit_timeout_and_failure() -> None:
    context = parse_knowledge_runtime_context(
        _snapshot(),
        tenant_id="tenant-a",
        task_id=7,
...
@pytest.mark.anyio
async def test_runtime_context_is_loaded_only_from_tenant_scoped_task_snapshot() -> None:
    calls = []
    ...
    owner_context = await DatabaseRuntimeProviderResourceResolver._resolve_knowledge_context(
        Session(),
        SimpleNamespace(
            tenant_id="tenant-a",
    ...
    assert owner_context.task_id == 7
