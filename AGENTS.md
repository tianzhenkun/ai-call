...
- 执行 SIP barge-in P1 专项任务时，范围限于本地快速打断门控；其他任务以用户当前明确目标为准，不自动套用 P1 范围。
- P2/P3 暂停，除非用户明确要求恢复。
...
- 用户描述的现象与提出的根因分开处理；定位根因时，关联同一次通话的实际生效配置、媒体/录音和事件时间线，证据不足时保留为待验证假设。
- 验证结论不得超出证据范围：服务健康、接口成功或回归测试通过，不能替代真实通话验收；响铃、接通、双向媒体、识别、打断和录音分别按实际证据报告。
