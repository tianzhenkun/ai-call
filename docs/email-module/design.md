...
创建弹窗调用时，设置先回写创建表单，最终随任务一并保存；现有任务调用时更新该任务。两处使用同一组件和校验规则。上层弹窗打开后，下层不可交互且保留输入；Esc 只关闭顶层，有未保存修改时提示。正文编辑与设置弹窗不继续叠出第三层完整表单。
...
| 待执行 | 已安排未来执行时间 | 查看、撤回定时 |
| 执行中 | 首封或后续跟进尚未结束 | 查看进度、停止任务 |
...
附件沿用参考界面的约束：最多 5 个、总计不超过 8 MB，支持 PDF、Office 文档和常见图片；前后端同时校验，发送前再次核对文件可读取及归属。
