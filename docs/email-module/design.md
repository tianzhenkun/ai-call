...
参照 Sales 编辑弹窗：主题（上限 512）、富文本正文、附件、AI 生成、中文审阅翻译、版本记录。任务模板不放单一收件邮箱选择器；收件人来自名单。签名在单独编辑入口维护，执行时合并到正文快照。
...
“AI 一键生成”产出待审阅的主题和正文，不直接发送；公司信息、任务上下文和允许的模板变量作为输入。中文审阅翻译使用同一套模型配置；版本恢复读取已保存内容，不调用模型。模型名称是用户选择的配置目标，不是已验证可调用的结论，实施联调需检查账户权限和接口兼容性。
...
回复编辑器支持空白一键生成、正文变量插入和中文审阅翻译。生成结合发件公司、当前客户及往来记录；无来信时不得暗示客户已回复或表达兴趣。变量来自当前客户的名单字段，提交时安全替换；单人回复/主动跟进中的缺失变量值需补全或删除变量后再提交。中文译文仅供审阅，不替换发送原文。所有 AI 结果均是草稿，不自动发送，过期结果不得覆盖人工编辑。

回复弹窗复用邮件模板的 AI 生成区、正文工具栏和翻译栏；附件在翻译栏前，翻译栏提供开关和说明，不设独立变量预览按钮，保留变量缺值提示。
