...
1. `stable_speech_candidate`：稳定人声候选；默认 180ms 先记录 candidate。若本地证据是中等音量、SNR 充足、包络平稳且不处在刚结束客户语音的尾音保护窗内，可直接进入 pre-stop；否则继续等到 pre-stop 授权时长后进入 pre-stop + clean window。
...
| `pre_stop_min_duration_ms` | `240` | 普通 `stable_speech_candidate` 拿到停播权限的最小时长；满足快速授权证据的中等平稳人声可在 180ms pre-stop |
...
-> 后续继续稳定到 pre_stop_min_duration_ms
-> sip_pre_stop
```

默认 `candidate_min_duration_ms=180`、`pre_stop_min_duration_ms=240`。这样 180ms 左右的短声可以被记录为 candidate，但不必一刀切：中等音量或清晰偏响、SNR 充足、未达到节律噪声包络阈值、非刚说完尾音窗口的本地人声可以快速 pre-stop；短促强声、热起声回落、低能量边缘声和刚结束客户语音后的同源尾音仍继续 deferred。
...
1. 180ms 平滑短声只产生 candidate/deferred，不直接 pre-stop。
