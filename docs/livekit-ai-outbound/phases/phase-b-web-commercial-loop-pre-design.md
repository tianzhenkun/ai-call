...
当前 Phase A 实现已收尾，验收报告见 [phase-a-acceptance-report.md](phase-a-acceptance-report.md)。本文档可以作为 Phase B 正式设计的输入，但不能直接作为实现依据。
...
3. 关键事件和指标持久化。
4. 通话结果查询。
5. 录音方案定稿，并在条件成熟时接入最小录音闭环。
6. 最小转人工状态或接管请求，不做完整坐席系统。
...
| 子阶段 | 目标 | 说明 |
|---|---|---|
| B1：记录与查询 | 通话记录、生效配置、事件和指标可持久化查询 | Phase B 最小必做 |
...
6. 生效配置字段：固定模型标识、实际音色、promptHash、openingMessageHash、配置文件中生效的 VAD 参数和浏览器音频约束。
...
| 对象 | 用途 |
|---|---|
| Call Record | 保存每通会话的开始、结束、状态、入口、Room、固定模型标识等摘要 |
| Effective Config | 保存创建会话时实际生效的固定模型、音色、promptHash、openingMessageHash、VAD 和音频约束；这些值来自配置文件，不代表模型、音色、开场白或调参管理能力 |
| Call Event | 保存关键运行事件 |
| Call Metrics | 保存首包、打断、播放、错误等指标 |
| Recording File | 保存录音文件索引，不在数据库保存音频内容 |
| Handoff Request | 保存转人工请求和处理状态 |

注意：这里不是表结构定稿。正式表结构必须等 Phase A 事件和指标稳定后再写。
...
当前可以进入 Phase B 正式技术设计，但不建议直接进入 Phase B 实现，也不建议立即定稿数据库表和录音方案。

正式设计前必须先复核 Phase A 验收报告中的补证项。补证项关闭后，再把本文档更新为 Phase B 正式技术设计。
