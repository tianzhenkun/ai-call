...
- 关键查询建立普通索引和业务唯一约束；
- 当前 `ai_call_record` 不保存 `tenant_id`，因此 V1 handoff 暂沿用既有 `000000` 单租户边界，查询不得为兼容而跨租户兜底；接入非 `000000` 宿主租户前，必须先补充上游权威租户来源并统一迁移通话与 handoff 链路。
...
`claim` 必须幂等。当前数据契约不新增通用幂等记录表，以 `(handoff_id, 当前登录坐席, console_session_id)` 作为认领重试身份；同一组合重复请求返回同一认领结果，其他坐席或标签页返回稳定冲突。成功只表示认领成功和媒体连接开始，不表示人工已接通。
...
1. 宿主系统新增菜单 `AI Call > 坐席工作台`，菜单权限标识为 `ai_call:agent:console`；通过现有角色与菜单授权控制前端页面入口，但 V1 Python 接口不校验该权限标识。
2. 宿主系统在 `AI Call` 下增加“坐席管理”“转人工记录”“跟进任务管理”三个管理菜单，V1 共用权限标识 `ai_call:agent:manage`，不再拆分细粒度管理权限；该标识只控制前端菜单展示，V1 管理接口仅校验登录，因此任意已登录用户均可访问。
3. 坐席管理页单独勾选四类 `scene_code`，保存到 `ai_call_agent_scene_scope`。菜单权限只决定前端是否展示入口，场景范围决定能看到和认领哪些任务，两者不能互相替代。
4. 普通坐席默认只查看有权限场景的待接任务、公共未接回访任务、自己的通话和认领后的本人跟进任务。
5. 服务端每次认领、查看和回拨都校验登录身份、坐席档案启用状态、`scene_code` 范围、任务状态及负责人，不能只依赖前端菜单隐藏。
6. V1 按 `LingChenAdmin` 现有方式只校验宿主登录身份，不接入宿主 permission code 集合；这是已知的权限边界。后续若需要收紧管理接口，可接入宿主真实菜单权限，而不改变坐席档案和场景授权模型。
